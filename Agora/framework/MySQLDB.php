...
class MySQL
{
    ...
    function __construct($host = "localhost", $dbUser = "root", $dbPass = "", $dbName = null)
    {
        $this->host = $host;
        $this->dbUser = $dbUser;
        $this->dbPass = $dbPass;
    ...
    function selectDatabase()
    {
        if (!mysqli_select_db($this->dbConn, $this->dbName)) {
            ...
            error_log("$this->dbName database selected");
        }
    }
    ...
    function insertRow($sql)
    {
        ...
        if ($aQResult) {
            ...
            print "Failed to insert the record<br>";
            return false;
        }
    }
    ...
    function execute($sql)
    {
        if (!($queryResource = mysqli_query($this->dbConn, $sql))) {
            $error = mysqli_error($this->dbConn);
            error_log("MySQL Execution Error: " . $error);
